...
from src.users.dependencies import get_db
...
def require_admin(db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
    try:
        ...
        if not user:
            raise UserNotFound('Admin no longer exist')
...
def require_payment(db: Session = Depends(get_db), Authorize: AuthJWT = Depends()):
    try:
        Authorize.jwt_required()
        user_id = Authorize.get_jwt_subject()
        user = db.query(User).filter(User.id ==
                                     user_id, User.role == 'payment').first()
